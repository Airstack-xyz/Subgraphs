import { Address, BigDecimal, BigInt, dataSource, Bytes, log } from "@graphprotocol/graph-ts";
import { AirAccount, AirContract, AirDailyAggregateEntity, AirDailyAggregateEntityAccount, AirDailyAggregateEntityStats, AirNFTSaleStats, AirNFTSaleTransaction, AirToken, AirTokenMetadata } from "../../../generated/schema";
import { ERC721MetaData } from "../../../generated/EthRegistrarController/ERC721MetaData"
...
export namespace nft {
    ...
    export function trackNFTSaleTransactions(
        ...
        timestamp: BigInt,
        blockNumber: BigInt,
        displayContentTypes: string[]  = [],
        displayContentValues: string[] = []
        ...
        for (let i = 0; i < transactionCount; i++) {
            ...
            sellActionDailyAggregatedEntity.blockHeight = blockNumber;
            ...
            buyActionDailyAggregatedEntity.blockHeight = blockNumber;
            ...
            // Transaction
            let displayContentType = displayContentTypes.length > i ? displayContentTypes[i]: null;
            let displayContentValue = displayContentValues.length > i ? displayContentValues[i]: null;
            ...
            if(displayContentType && displayContentValue) {
                let tokenMetaData = getOrCreateTokenMetaData(transaction.id, displayContentType, displayContentValue);
                transaction.tokenMetadata = tokenMetaData.id;
                tokenMetaData.save();
            }
    ...
    export function getOrCreateTokenMetaData(id: string, displayContentType: string, displayContentValue: string): AirTokenMetadata {

        let airTokenMetadata = AirTokenMetadata.load(id);

        if (airTokenMetadata == null) {
            airTokenMetadata = new AirTokenMetadata(id);
            airTokenMetadata.displayContentType = displayContentType;
            airTokenMetadata.displayContent = displayContentValue;
        }

        return airTokenMetadata as AirTokenMetadata;
    }
