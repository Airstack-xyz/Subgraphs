    ...
    writeSubgraphYaml(vertical as Vertical, yamlPath, dataSources, templates)
      .then(() => {
        writeSubgraphGraphql(vertical as Vertical, graphql)
          .then(() => {
            const { targetDir, commontTargetDir } = copyAirstackModules(
            ...
            if (dataSources && dataSources.length > 0) {
              ...
              try {
                const sourceSubgraphYaml = yaml.load(yamlSchemas) as Record<
                  string,
                  any
                >
                if (
                  sourceSubgraphYaml.dataSources &&
                  sourceSubgraphYaml.dataSources.length > 0
                ) {
                  dataSourceName = sourceSubgraphYaml.dataSources[0].name
                } else if (
                  sourceSubgraphYaml.templates &&
                  sourceSubgraphYaml.templates.length > 0
                ) {
            ...
            arrayOfFiles.forEach((filePath: string) => {
              writeFilePromise.push(
                new Promise((res, rej) => {
                  ...
                  const finalFileContent = mustache.render(fileContent, {
            ...
            Promise.all(writeCommonFilesPromise)
              .then(() => {
                resolve()
              })
              .catch(() => {
  ...
  return new Promise((resolve, reject) => {
    ...
    targetDataSources.forEach((dataSrc: Record<string, any>) => {
      if (whiteListedDataSource!.includes(dataSrc.name)) {
        ...
        const existingAbiNames = dataSrc.mapping.abis.map(
          (abiObj: Record<string, string>) => {
            return abiObj.name
          }
        )
      }
...
function writeSubgraphGraphql(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (sourceSchemas.includes(schemasComment)) {
      ...
      const sourceSchemasAboveComment = sourceSchemas.substring(
        0,
        indexOfComment
      )
      // overwrite the file with the schema above the comment
      const isNonAirstackSchemaWriteSuccess = Utils.overwriteFile(
        schemaGraphqlPath,
        sourceSchemasAboveComment
      )
      ...
      const isAppendSuccess = await Utils.appendFiles(
        schemaGraphqlPath,
        schemas
      )
      if (!isAppendSuccess) {
        console.error(
          "Failed to append airstack schemas to schema.graphql file after removing the existing airstack schemas"
        )
        reject()
      }
      return resolve()
    ...
    if (!isBackupSuccess) {
      console.error(
        "Failed to backup schema.graphql file to schema.graphql.bck file"
      )
      reject()
    }
...
function copyAirstackModules(
  vertical: Vertical
): { targetDir: string; commontTargetDir: string } {
  ...
  switch (vertical) {
    case Vertical.NftMarketplace:
      sourceDir = path.resolve(
        __dirname,
        "../../modules/airstack/nft-marketplace"
      )
  ...
  const commontSourceDir = path.resolve(
    __dirname,
    "../../modules/airstack/common"
  )
  const commontTargetDir = path.resolve(
    __dirname,
    "../../../../../modules/airstack/common"
  )
...
function createConfigFile() {
  // Define the content you want in the JSON file
  const configData = {
    name: "",
    version: "",
    slug: "",
  ...
  const jsonData = JSON.stringify(configData, null, 2) // null and 2 for pretty formatting

  // Write the JSON string to the config.json file
...
function createTemplateFile() {
  // Define the content you want in the JSON file
  const configData = `
  export const SUBGRAPH_NAME = "{{name}}"
  export const SUBGRAPH_VERSION = "{{version}}"
  export const SUBGRAPH_SLUG = "{{slug}}"
  `

  // Write the JSON string to the config.json file
  fs.writeFileSync("meta.template", configData)
}
