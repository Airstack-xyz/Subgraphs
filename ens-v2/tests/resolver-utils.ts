...
export function getAddrChangedEvent(input: AddrChangedInput): AddrChanged {
    ...
    let a = new ethereum.EventParam("a", ethereum.Value.fromAddress(Address.fromString(input.a)))
...
export function getAddressChangedEvent(input: AddressChangedInput): AddressChanged {
    // preparing event params
    let node = new ethereum.EventParam(
        "node",
        ethereum.Value.fromBytes(Bytes.fromHexString(input.node))
    ...
    let newAddress = new ethereum.EventParam(
        "newAddress",
        ethereum.Value.fromAddress(Address.fromString(input.newAddress))
    )
...
export function getTextChangedWithValueEvent(input: TextChangedWithValueInput): TextChanged1 {
    // preparing event params
    let node = new ethereum.EventParam(
        "node",
        ethereum.Value.fromBytes(Bytes.fromHexString(input.node))
    ...
    let key = new ethereum.EventParam("key", ethereum.Value.fromString(input.key))
    let value = new ethereum.EventParam("value", ethereum.Value.fromString(input.value))
    // initializing event variable
