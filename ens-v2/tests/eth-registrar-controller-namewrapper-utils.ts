...
import {
    ...
    _handleNameRenewed,
} from "../src/eth-registrar-controller-namewrapper"
...
export function mockNameRegistered(input: NameRegisteredInput): void {
    ...
    const baseCost = BigInt.fromString(input.baseCost)
    const premium = BigInt.fromString(input.premium)
...
export function mockNameRenewed(input: NameRenewedInput): void {
    ...
    _handleNameRenewed(controller, txHash, block, name, label, cost, expires, logIndex, from)
}
