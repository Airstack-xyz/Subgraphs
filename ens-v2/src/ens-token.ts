...
export function _handleControllerAdded(txHash: Bytes, controller: Address): void {
...
export const _handleNameRegistered = (
    ...
    airstack.domain.trackAirDomainRegistrationExpiry(
        tokenAddress,
        id,
...
export function handleNameRegistered1(event: NameRegistered1): void {
    ...
    _handleNameRegistered(tokenAddress, txHash, block, id, expires, owner, event.logIndex)
}
...
export function handleNameRenewed1(event: NameRenewed1): void {
    ...
    const tokenAddress = event.address
    _handleNameRenewed(tokenAddress, txHash, from, block, id, expires, event.logIndex)
}
...
export function handleTransfer1(event: Transfer1): void {
    ...
    const tokenAddress = event.address
    _handleTransfer(tokenAddress, txHash, block, from, to, tokenId, event.logIndex)
}
